        ...
        <>
            <div className={`flex flex-col gap-3 pb-5 ${active ? "active" : ""}`}>
                <div className="flex justify-between cursor-pointer" onClick={onToggle}>
                    <p className={`worksans-500 text-[15px] duration-200 ${active ? "text-amber-500" : ""}`}>{question}</p>
                ...
                <p className="answer_wrapper text-slate-500 text-[15px] worksans pr-20" ref={contentEl} style={ active ? { height: contentEl.current.scrollHeight } : { height: "0px" }}>{answer}</p>
            </div>
        </>
    )
}
